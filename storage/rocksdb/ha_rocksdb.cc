...
static PSI_memory_key bh_key_memory_rocksdb_share;
...
static mysql_mutex_t rocksdb_mutex;
static HASH rocksdb_open_tables;

static st_rocksdb_share *get_share(const char *table_name);
static void free_share(st_rocksdb_share *share);

/*****************************************************************************
** rocksdb tables
*****************************************************************************/

ha_rocksdb::ha_rocksdb(handlerton *hton,
...
{
  DBUG_ENTER("ha_rocksdb::truncate");
  DBUG_RETURN(0);
}

const char *ha_rocksdb::index_type(uint key_number)
{
  DBUG_ENTER("ha_rocksdb::index_type");
...
{
  int rc;
  DBUG_ENTER("ha_rocksdb::index_read");
...
int ha_rocksdb::index_read_idx_map(uchar * buf, uint idx, const uchar * key,
                                 key_part_map keypart_map,
                                 enum ha_rkey_function find_flag)
{
  int rc;
  DBUG_ENTER("ha_rocksdb::index_read_idx");
  MYSQL_INDEX_READ_ROW_START(table_share->db.str, table_share->table_name.str);
  THD *thd= ha_thd();
  if (is_slave_applier(thd) && thd->query().str == NULL)
    rc= 0;
...
{
  int rc;
  DBUG_ENTER("ha_rocksdb::index_read_last");
  MYSQL_INDEX_READ_ROW_START(table_share->db.str, table_share->table_name.str);
  THD *thd= ha_thd();
  if (is_slave_applier(thd) && thd->query().str == NULL)
    rc= 0;
...
{
  int rc;
  DBUG_ENTER("ha_rocksdb::index_prev");
  MYSQL_INDEX_READ_ROW_START(table_share->db.str, table_share->table_name.str);
  rc= HA_ERR_END_OF_FILE;
  MYSQL_INDEX_READ_ROW_DONE(rc);
  table->status= STATUS_NOT_FOUND;
...
static st_rocksdb_share *get_share(const char *table_name)
{
  st_rocksdb_share *share;
  uint length;

  length= (uint) strlen(table_name);
  mysql_mutex_lock(&rocksdb_mutex);

  if (!(share= (st_rocksdb_share*)
        my_hash_search(&rocksdb_open_tables,
                       (uchar*) table_name, length)))
  {
    if (!(share= (st_rocksdb_share*) my_malloc(bh_key_memory_rocksdb_share,
                                                 sizeof(st_rocksdb_share) +
                                                 length,
...
static void rocksdb_free_key(st_rocksdb_share *share)
{
  thr_lock_delete(&share->lock);
  my_free(share);
}

static uchar* rocksdb_get_key(st_rocksdb_share *share, size_t *length,
...
static PSI_mutex_key bh_key_mutex_rocksdb;

static PSI_mutex_info all_rocksdb_mutexes[]=
{
  { &bh_key_mutex_rocksdb, "rocksdb", PSI_FLAG_GLOBAL}
};

static PSI_memory_info all_rocksdb_memory[]=
{
  { &bh_key_memory_rocksdb_share, "rocksdb_share", 0}
};
...
static int rocksdb_init(void *p)
{
  ...
  rocksdb_hton->flags= HTON_CAN_RECREATE | HTON_SUPPORTS_ONLINE_BACKUPS;

  mysql_mutex_init(bh_key_mutex_rocksdb,
                   &rocksdb_mutex, MY_MUTEX_INIT_FAST);
  (void) my_hash_init(&rocksdb_open_tables, system_charset_info,32,0,0,
                      (my_hash_get_key) rocksdb_get_key,
                      (my_hash_free_key) rocksdb_free_key, 0,
                      bh_key_memory_rocksdb_share);

  return 0;
}

static int rocksdb_fini(void *p)
{
...
mysql_declare_plugin(rocksdb)
{
  ...
  PLUGIN_LICENSE_GPL,
  rocksdb_init, /* Plugin Init */
  rocksdb_fini, /* Plugin Deinit */
  0x0100 /* 1.0 */,
