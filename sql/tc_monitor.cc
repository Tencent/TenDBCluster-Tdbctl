...
2.log result in tdbctl: mysql.cluster_heartbeat_log
*/
int tc_check_cluster_availability()
{
	//init for sql
	string check_heartbeat_sql = "update cluster_monitor.cluster_heartbeat set k=(k+1)%1024";
	string heartbeat_log_sql_pre = "replace into mysql.cluster_heartbeat_log( "
	...
	{
		MYSQL_RES* res;
		string sql_max_id = "select max(id) from mysql.cluster_heartbeat_log";
		MYSQL_ROW row = NULL;
...
tc_check_cluster_availability do check work and log in mysql.cluster_heartbeat_log
*/
void tc_check_cluster_availability_thread()
{
	while (1)
	{
		...
		{
			for (ulong i = 0; i < tc_check_availability_interval; ++i)
			{
				sleep(1);
			}
