...
bool Session_rules_resource_mgr::init(char **var) {
  if(*var) {
    char *val = NULL;
    if(val = (char*) my_memdup(key_memory_Sys_var_charptr_value,
                                *var, strlen(*var)+1, MYF(MY_WME))) {
      *var = val;
      m_charptr_set.insert(*var);
    } else {
      return false;  /* Error */
    }
  }
...
bool Session_rules_resource_mgr::update(char **var, char *val, size_t val_len) {
  char *new_val = NULL;
  if (val)
  {
...
void Session_rules_resource_mgr::deinit() {
  char *charptr;
  for(charptr: m_charptr_set) {
    if(charptr) {
      my_free(charptr);
    }
  }
...
void Session_rules_resource_mgr::claim_memory_ownership() {
  char *charptr;
  for(charptr: m_charptr_set) {
    if(charptr) {
      my_claim(charptr);
  ...
  // For m_charptr_set, there is no good way to claim memory ownership
}