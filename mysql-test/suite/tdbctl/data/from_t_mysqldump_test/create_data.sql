...
CREATE DATABASE mysqldump_test_db1;
USE mysqldump_test_db1;

SET @old_innodb_file_per_table=@@GLOBAL.innodb_file_per_table;
set global innodb_file_per_table=1;
/* CREATE TABLE t1(a INT, KEY (a)) ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=1 ENGINE=Innodb; */
CREATE TABLE t1(a INT not null, KEY (a)) ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=1 ENGINE=Innodb;
INSERT INTO t1 VALUES (1), (2);
SET GLOBAL innodb_file_per_table=@old_innodb_file_per_table;

/* CREATE TABLE t2 (a decimal(64, 20)); */
CREATE TABLE t2 (a decimal(64, 20) primary key);
...
INSERT IGNORE INTO t3 VALUES ('-9e999999');

/* CREATE TABLE t4 (a DECIMAL(10,5), b FLOAT); */
CREATE TABLE t4 (a DECIMAL(10,5), b FLOAT not null, key (b));
INSERT INTO t4 VALUES (1.2345, 2.3456);
INSERT INTO t4 VALUES ('1.2345', 2.3456);
INSERT INTO t4 VALUES ("1.2345", 2.3456);
...
/* CREATE TABLE t5(a int, b text, c varchar(3)); */
CREATE TABLE t5(a int primary key, b text, c varchar(3));
INSERT INTO t5 VALUES (1, "test", "tes"), (2, "TEST", "TES");

/* CREATE TABLE t6 (`a"b"` char(2)); */
CREATE TABLE t6 (`a"b"` char(2) primary key);
INSERT INTO t6 VALUES ("1\""), ("\"2");

/* CREATE TABLE t7 (a  VARCHAR(255)) DEFAULT CHARSET koi8r; */
CREATE TABLE t7 (a  VARCHAR(255) primary key) DEFAULT CHARSET koi8r;
INSERT INTO t7  VALUES (_koi8r x'C1C2C3C4C5'), (NULL);
...
create database mysqldump_test_db2 character set latin2 collate latin2_bin;

/* CREATE TABLE t11 (a  CHAR(10)); */
CREATE TABLE t11 (a  CHAR(10) primary key);
INSERT INTO t11  VALUES (_latin1 '����');
...
/* CREATE TABLE t14 (`b` blob);
INSERT INTO `t14` VALUES (0x602010000280100005E71A); */
CREATE TABLE `t14` (`b` blob, a int not null, key (a));
INSERT INTO `t14` VALUES (0x602010000280100005E71A, 20250923);

/* CREATE TABLE t15 (a INT); */
CREATE TABLE t15 (a INT primary key);
INSERT INTO t15 VALUES (1),(2),(3);
INSERT INTO t15 VALUES (4),(5),(6);
...
create table t16 (
 ...
 F_6faa8040da20ef399b63a72d0e4ab575 int,
 F_fe73f687e5bc5280214e0486b273a5f9 int, 
 F_key int primary key auto_increment);
insert into t16 (F_8d3bba7425e7c98c50f52ca1b52d3735) values (1);

/* CREATE TABLE t17 (a int); */
CREATE TABLE t17 (a int primary key);
INSERT INTO t17 VALUES (1),(2),(3);

CREATE DATABASE mysqldump_test_db3;
USE mysqldump_test_db3;
...
create database mysqldump_test_db4;
use mysqldump_test_db4;
...
/* create table t18 (a int(10)); */
create table t18 (a int(10), b int primary key auto_increment);
create table t19 (pk int primary key auto_increment,
a int(10), b varchar(30), c datetime, d blob, e text);
/* insert into t18 values (NULL), (10), (20); */
insert into t18(a) values (NULL), (10), (20);
insert into t19 (a, b) values (NULL, NULL),(10, NULL),(NULL, "twenty"),(30, "thirty");

/* create table t20 (a text character set utf8, b text character set latin1);
insert t20 values (0x4F736E616272C3BC636B, 0x4BF66C6E); */
create table t20 (a text, b text, c int primary key auto_increment);
insert t20(a, b) values (0x4F736E616272C3BC636B, 0x4BF66C6E);
...
insert into t21 (t1_name) values('bla');
insert into t21 (t1_name) values('bla');
insert into t21 (t1_name) values('bla');
...
insert into t22 values (0815, 4711, 2006);

create database mysqldump_test_db6;
use mysqldump_test_db6;
/* create table t1 (a int); */
create table t1 (a int primary key);
...
create database mysqldump_test_db7;
use mysqldump_test_db7;
...
/* CREATE TABLE t23 (a char(10)); */
CREATE TABLE t23 (a char(10) primary key);
INSERT INTO t23 VALUES ('\'');

/* create table t24(a int, b int, c varchar(30)); */
create table t24(a int, b int, c varchar(30), primary key(a));
insert into t24 values(1, 2, "one"), (2, 4, "two"), (3, 6, "three");

create view v26 as
select * from t24;

create view v24 as
select * from v26 where b in (1, 2, 3, 4, 5, 6, 7);

create view v25 as
select v26.a from v26, v24 where v24.a=v26.a and v26.b=3 limit 1;

/* CREATE TABLE t25 (a int, b bigint default NULL);
CREATE TABLE t26 (a int); */
CREATE TABLE t25 (a int, b bigint default NULL, c int primary key auto_increment);
CREATE TABLE t26 (a int, c int primary key auto_increment);
delimiter |
create trigger trg1 before insert on t25 for each row
begin
  if new.a > 10 then
    set new.a := 10;
    set new.a := 11;
...
create trigger trg2 before update on t25 for each row begin
  if old.a % 2 = 0 then set new.b := 12; end if;
end|
set sql_mode="traditional"|
create trigger trg3 after update on t25 for each row
begin
  if new.a = -1 then
    set @fired:= "Yes";
  end if;
end|
create trigger trg4 before insert on t26 for each row
begin
  if new.a > 10 then
    set @fired:= "No";
  end if;
...
INSERT INTO t25 (a) VALUES (1),(2),(3),(22);
update t25 set a = 4 where a=3;

/* CREATE TABLE `test1` (
  `a1` int(11) default NULL
) ENGINE=MyISAM DEFAULT CHARSET=latin1; */
CREATE TABLE `test1` (
  `a1` int(11) PRIMARY KEY
) ENGINE=MyISAM DEFAULT CHARSET=latin1;
...
BEGIN
  select sum(id) from id_list into a;
END //
...
create table t27 (`d` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, unique (`d`));
set time_zone='+00:00';
insert into t27 values ('2003-10-25 22:00:00'),('2003-10-25 23:00:00');

SELECT @@SESSION.tc_admin INTO @old_tc_admin;
SET @@SESSION.tc_admin = OFF;

/*
  Spider: (Error 12504: Different multiple table link parameter's count)
*/
...
SET @@SESSION.tc_admin = @old_tc_admin;

/* create table t28 (a int, b varchar(32), c varchar(32)); */
create table t28 (a int primary key, b varchar(32), c varchar(32));
insert into t28 values (1, 'first value', 'xxxx');
insert into t28 values (2, 'second value', 'tttt');
insert into t28 values (3, 'third value', 'vvv vvv');

create view v28 as select * from t28;
create view v27 as select * from v28;
create view v29 as select * from v27;

SET @old_sql_mode = @@SQL_MODE;
SET SQL_MODE = IGNORE_SPACE;
/* CREATE TABLE t29 (a INT); */
CREATE TABLE t29 (a INT, b INT PRIMARY KEY);
DELIMITER |
CREATE TRIGGER tr29 BEFORE INSERT ON t29
       FOR EACH ROW
       BEGIN
         SET new.a = 0;
       END|
...
/* create table t30 (a binary(1), b blob); */
create table t30 (a binary(1) primary key, b blob);
insert into t30 values ('','');

/* create table t31 (a int); */
create table t31 (a int primary key);
insert into t31 values (289), (298), (234), (456), (789);
create definer = CURRENT_USER view v31 as select * from t31;
create SQL SECURITY INVOKER view v32 as select * from t31;
create view v33 as select * from t31 with local check option;
create algorithm=merge view v34 as select * from t31 with cascaded check option;
create algorithm=temptable view v35 as select * from t31;

/* create table t32 (a int, created datetime);
create table t33 (b int, created datetime); */
create table t32 (a int primary key, created datetime);
create table t33 (b int primary key, created datetime);
create trigger tr32 before insert on t32 for each row set
new.created=now();
delimiter |
create trigger tr33 after insert on t32
for each row
begin
  insert into t33 set b=new.a and created=new.created;
end|
delimiter ;

/* create table t35 (qty int, price int); */
create table t35 (qty int primary key, price int);
insert into t35 values(3, 50);
insert into t35 values(5, 51);
create view v1_t35 as select qty, price, qty*price as value from t35;
create view v2_t35 as select qty from v1_t35;

DELIMITER |
/*!50003 CREATE FUNCTION `f36`() RETURNS bigint(20)
DETERMINISTIC
return 42 */|
/*!50003 CREATE PROCEDURE `p36`()
DETERMINISTIC
select 42 */|
DELIMITER ;

create table t37 ( id serial );
create view v37 as select * from t37;

create database mysqldump_test_db8;
use mysqldump_test_db8;
...
create table t1 (f1 int not null, f2 int not null, INDEX (f1, f2));
...
/* CREATE TABLE t38 (f1 int(10), data MEDIUMBLOB); */
CREATE TABLE t38 (f1 int(10) primary key, data MEDIUMBLOB);
INSERT INTO t38 VALUES(1,0xff00fef0);

/* CREATE TABLE t40 (a INT) ENGINE=MYISAM;
CREATE TABLE t41 (a INT) ENGINE=MYISAM;
CREATE TABLE t39 (a INT) ENGINE=merge UNION=(t40, t41); */
CREATE TABLE t40 (a INT PRIMARY KEY) ENGINE=MYISAM;
CREATE TABLE t41 (a INT PRIMARY KEY) ENGINE=MYISAM;
/* MERGE engine is not supported */
/* CREATE TABLE t39 (a INT PRIMARY KEY) ENGINE=merge UNION=(t40, t41); */

create database mysqldump_bug23491_original;
use mysqldump_bug23491_original;
...
-- SPIDER BUG: Spider: (Error 1305: FUNCTION mysqldump_bug23491_original.f1 does not exist)
/* create view v2 as select f1(); */ 
/* create function f2() returns int return f1(); */
create function f2() returns int DETERMINISTIC return f1();
/* create view v3 as select mysqldump_bug23491_original.f1(); */

create database mysqldump_test_db9;
create procedure mysqldump_test_db9.sp1() select 'hello';

USE mysqldump_test_db1;

/* CREATE TABLE t42 (c1 INT, c2 LONGBLOB); */
CREATE TABLE t42 (c1 INT PRIMARY KEY, c2 LONGBLOB);
INSERT INTO t42 SET c1=11, c2=REPEAT('q',509);

CREATE VIEW v42 AS SELECT 1;

/* CREATE TABLE t43 (c1 INT); */
CREATE TABLE t43 (c1 INT PRIMARY KEY);
CREATE TRIGGER t43bd BEFORE DELETE ON t43 FOR EACH ROW BEGIN END;

CREATE TABLE t44 (c1 INT NOT NULL AUTO_INCREMENT PRIMARY KEY);

SET @TMP_SQL_MODE = @@SQL_MODE;
SET SQL_MODE = 'NO_AUTO_VALUE_ON_ZERO';
-- SPIDER BUG: Spider: (Error 1062: Duplicate entry '0' for key 'PRIMARY')
/* INSERT INTO t44 VALUES (0), (1), (2); */
SET SQL_MODE = @TMP_SQL_MODE;

create database mysqldump_db42635;
use mysqldump_db42635;
/* create table t1 (id int); */
create table t1 (id int primary key);
create view mysqldump_db42635.v1 (c) as select * from mysqldump_db42635.t1;
create view mysqldump_db42635.v2 (c) as select * from mysqldump_db42635.t1;
...
/* TDBCTL BUG: The "TDBCTL CHECK TABLES" command ignores the charset and causes the matching error */
/*
CREATE TABLE `straße` ( f1 INT PRIMARY KEY);
CREATE TABLE `כדשגכחךלדגכחשךדגחכךלדגכ` ( f1 INT PRIMARY KEY);
*/
SET NAMES latin1;

/* CREATE TABLE t55(a int, b int); */
CREATE TABLE t55 (a int not null, b int not null, index (a, b));
INSERT INTO t55 VALUES (1,1);
INSERT INTO t55 VALUES (2,3);
INSERT INTO t55 VALUES (3,4), (4,5);

/* create table t56 (a text , b text);
create table t57 (a text , b text); 
insert t56 values ("Duck, Duck", "goose");
insert t56 values ("Duck, Duck", "pidgeon");
insert t57 values ("We the people", "in order to perform");
insert t57 values ("a more perfect", "union"); */
create table t56 (a text , b text, c int primary key auto_increment);
create table t57 (a text , b text, c int primary key auto_increment); 
insert t56(a, b) values ("Duck, Duck", "goose");
insert t56(a, b) values ("Duck, Duck", "pidgeon");
insert t57(a, b) values ("We the people", "in order to perform");
insert t57(a, b) values ("a more perfect", "union"); 

/* create table words(a varchar(255));
create table words2(b varchar(255)); */
create table words (a varchar(255) primary key);
create table words2 (b varchar(255) primary key);

create database mysqldump_first;
use mysqldump_first;
/* TDBCTL: Unknown or incorrect time zone: 'UTC' */
/* set time_zone = 'UTC'; */
set time_zone = '+0:00';
...
create database mysqldump_test_db10;
use mysqldump_test_db10;
/* create table t1 (id int); */
create table t1 (id int primary key);
create view v1 as select * from t1;
insert into t1 values (1232131);
...
CREATE DATABASE mysqldump_test_db11;

set names koi8r;
-- BUG: unrecognized filed name
/* CREATE VIEW mysqldump_test_db11.v2 AS SELECT 1 AS �������1;
CREATE VIEW mysqldump_test_db11.v1 AS SELECT �������1 FROM mysqldump_test_db11.v2; */
CREATE VIEW mysqldump_test_db11.v2 AS SELECT 1 AS test;
CREATE VIEW mysqldump_test_db11.v1 AS SELECT test FROM mysqldump_test_db11.v2; 
set names latin1;
...
create database `mysqldump-test-database1`;
use `mysqldump-test-database1`;
/* create table test (a int); */
create table test (a int primary key);

CREATE DATABASE mysqldump_test_db12;
use mysqldump_test_db12;
CREATE VIEW v1(x, y) AS SELECT 'a', 'a';

use mysqldump_test_db1;

/* create table t58 (a int); */
create table t58 (a int, b int primary key);
create view v58 as select a from t58;

/* create table `load` (a varchar(255)); */
create table `load` (a varchar(255) primary key);

SET @@GLOBAL.CONCURRENT_INSERT = @OLD_CONCURRENT_INSERT;


/* CREATE TABLE t59 (f1 INT); */
CREATE TABLE t59 (f1 INT, f2 INT PRIMARY KEY AUTO_INCREMENT);
CREATE TRIGGER tr59 BEFORE UPDATE ON t59 FOR EACH ROW SET @f1 = 1;
CREATE PROCEDURE pr59 () SELECT "Meow";
CREATE EVENT ev59 ON SCHEDULE AT '2030-01-01 00:00:00' DO SELECT "Meow";

SET NAMES utf8;
/* CREATE TABLE t60 (a INT, b CHAR(10) CHARSET koi8r, c CHAR(10) CHARSET latin1); */
CREATE TABLE t60 (a INT PRIMARY KEY, b CHAR(10), c CHAR(10));
CREATE TABLE t61 LIKE t60;
INSERT INTO t60 VALUES (1, 'ABC-���', 'DEF-���'), (2, NULL, NULL);
SET NAMES default;

/* CREATE TABLE t62 (a BLOB) CHARSET latin1; */
CREATE TABLE t62 (a BLOB, b INT PRIMARY KEY) CHARSET latin1;
CREATE TABLE t62_copy LIKE t62;

/* create table t63 (first char(28) , last varchar(37)); */
CREATE TABLE t63 (first char(28) primary key, last varchar(37));
insert into t63 values ("Magnus", "Bl�udd");

/* TDBCTL: CREATE TABLE FAILED: ERROR: UNSUPPORTED SQL CREATE TABLE WITH TABLE COMMENT */
/* CREATE TABLE `comment_table` (i INT COMMENT 'FIELD COMMENT') COMMENT = 'TABLE COMMENT'; */
CREATE TABLE `comment_table` (i INT COMMENT 'FIELD COMMENT', primary key (i));

CREATE DATABASE `mysqldump-test-database2`;
USE `mysqldump-test-database2`;
/* CREATE TABLE `test` (`c1` VARCHAR(10)) ENGINE=MYISAM DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci; */
CREATE TABLE `test` (`c1` VARCHAR(10) PRIMARY KEY) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
ALTER DATABASE `mysqldump-test-database2` CHARACTER SET latin1 COLLATE latin1_swedish_ci;
ALTER DATABASE `mysqldump-test-database2` CHARACTER SET utf8 COLLATE utf8_unicode_ci ;

CREATE DATABASE mysqldump_BUG52792_1;
USE mysqldump_BUG52792_1;
/* CREATE TABLE t1 (c1 INT, c2 VARCHAR(20)) ENGINE=MyISAM;
CREATE TABLE t2 (c1 INT) ENGINE=MyISAM; */
CREATE TABLE t1 (c1 INT PRIMARY KEY, c2 VARCHAR(20)) ENGINE=MyISAM;
...
CREATE EVENT e1 ON SCHEDULE EVERY 1 SECOND DO DROP DATABASE mysqldump_BUG52792_1;
CREATE EVENT e2 ON SCHEDULE EVERY 1 SECOND DO DROP DATABASE mysqldump_BUG52792_1;
...
CREATE DATABASE mysqldump_BUG52792_2;
USE mysqldump_BUG52792_2;
SET NAMES utf8;
/* CREATE FUNCTION `stra�e` ( c1 CHAR(20))
  RETURNS CHAR(50) DETERMINISTIC
RETURN CONCAT(']]>, ', s, '!'); */
SET NAMES default;

USE `mysqldump_test_db1`;
/* CREATE TABLE t64 (a int, b int); */
CREATE TABLE t64 (a int primary key, b int);
CREATE TRIGGER tt64_t64 BEFORE INSERT ON t64 FOR EACH ROW 
  SET NEW.b=NEW.a + 10;

INSERT INTO t64 (a) VALUES (1),(2),(3);

/* CREATE TABLE t65 (a INT); */
CREATE TABLE t65 (a INT PRIMARY KEY);
...
CREATE DATABASE mysqldump_b12809202_db;
/* CREATE TABLE mysqldump_b12809202_db.t1 (c1 INT);
CREATE TABLE mysqldump_b12809202_db.t2 (c1 INT); */
CREATE TABLE mysqldump_b12809202_db.t1 (c1 INT PRIMARY KEY);
CREATE TABLE mysqldump_b12809202_db.t2 (c1 INT PRIMARY KEY);

INSERT INTO mysqldump_b12809202_db.t1 VALUES (1), (2), (3);
INSERT INTO mysqldump_b12809202_db.t2 VALUES (1), (2), (3);

/* CREATE DATABASE `mysqldump_a\\k`; */
/* CREATE TABLE `mysqldump_a\\k`.t1(i INT); */
/* TDBCTL BUG, wait to be fixed */
/* Remote: (Error 1049: Unknown database 'mysqldump_a\\k_0') */
/* CREATE TABLE `mysqldump_a\\k`.t1(i INT PRIMARY KEY); */

CREATE DATABASE mysqldump_dump_gis;
USE mysqldump_dump_gis;
/* CREATE TABLE t1 (a GEOMETRY);
INSERT INTO t1 VALUES(ST_GeomFromText('LineString(1 1, 2 1, 2 2, 1 2, 1 1)')); */
/* Spider: (Error 1178: The storage engine for the table doesn't support GEOMETRY) */
/* CREATE TABLE t1 (a GEOMETRY, b INT PRIMARY KEY AUTO_INCREMENT);
INSERT INTO t1(a) VALUES(ST_GeomFromText('LineString(1 1, 2 1, 2 2, 1 2, 1 1)')); */

CREATE DATABASE mysqldump_db_20772273;
USE mysqldump_db_20772273;
/* CREATE TABLE t1(a INT); */
CREATE TABLE t1(a INT PRIMARY KEY);
INSERT INTO t1 VALUES (1), (2);
/* CREATE TABLE t2(a INT); */
...
CREATE DATABASE mysqldump_dump_json;
USE mysqldump_dump_json;
/* CREATE TABLE t1 (j JSON);
INSERT INTO t1 VALUES (JSON_ARRAY(1, 2, 3, "one", "two", "three")); */
CREATE TABLE t1 (j JSON, pk INT PRIMARY KEY AUTO_INCREMENT);
INSERT INTO t1(j) VALUES (JSON_ARRAY(1, 2, 3, "one", "two", "three"));

CREATE DATABASE mysqldump_dump_generated;
USE mysqldump_dump_generated;

/* TDBCTL: CREATE TABLE FAILED: ERROR: too many key more than 1, but without unique key or set shard key */
...
CREATE DATABASE mysqldump_bug25717383;
use mysqldump_bug25717383;

/* CREATE TABLE `tab
one` (a int); */
CREATE TABLE `tab
one` (a int primary key);
...
/* Spider: (Error 1562: Cannot create temporary table with partitions) */
/* CREATE TEMPORARY TABLE `temp
one` (id INT PRIMARY KEY); */
